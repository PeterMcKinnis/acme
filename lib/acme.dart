...
import 'dart:convert' show LineSplitter, base64Decode, base64UrlEncode, json, jsonDecode, jsonEncode, utf8;
import 'dart:io' show HttpClient, HttpClientResponse;
import 'dart:typed_data' show Uint8List;
import 'package:basic_utils/basic_utils.dart'
    show AsymmetricKeyPair, CryptoUtils, DnsUtils, RRecordType, RSAPrivateKey, RSAPublicKey, X509Utils;
...
Future<AcmeChallengeResult> acmeDns01Challenge(
    {required List<String> hosts,
    required Future<dynamic> Function(String name, String value) createDnsTxtRecord,
  ...
  return _withCleanup((defer) async {
    ...
    if (account == null) {
      ...
      account = await client.newAccount(email: email!, termsOfServiceAgreed: termsOfServiceAgreed);
    }
    ...
    log("success");
    return AcmeChallengeResult(account: account!, privatePem: privatePem, publicPem: publicPem);
  });
}
...
Future<void> _doDnsChallenge(
  ...
  return _withCleanup((defer) async {
    ...
    var challenge = authorization.challenges.singleWhere((e) => e.type == "dns-01");
    ...
    var recordInfo = await createDnsTxtRecord(challengeRecord.name, challengeRecord.value);
    defer(() async {
    ...
    var i = 0;
    while (true) {
      var records = await DnsUtils.lookupRecord(challengeRecord.name, RRecordType.TXT);
      if (records != null && records.any((e) => e.data == challengeRecord.value)) {
        log("$prefix - public DNS TXT record found");
        break;
      }
      i += 1;
      if (i == maxDnsLookupRetrys) {
        throw Exception("$prefix - did not find public DNS record in $maxDnsLookupRetrys tries, aborting");
      }
    ...
    while (true) {
      ...
      if (status == AcmeStatus.valid) {
        ...
        if (i == maxAcmeCheckRetrys) {
          throw Exception("$prefix - challenge status pending after $maxAcmeCheckRetrys tries, aborting");
        }
...
class AcmeChallengeResult {
  AcmeChallengeResult({required this.account, required this.privatePem, required this.publicPem});
  AcmeAccount account;
  String privatePem;
  String publicPem;
}
...
class AcmeClient {
  ...
  static Future<AcmeClient> createLetsEncryptStaging() => create(letsEncryptStagingDirectoryUrl);
  static Future<AcmeClient> createLetsEncrypt() => create(letsEncryptDirectoryUrl);
  ...
  Future<AcmeAccount> newAccount({required String email, required bool termsOfServiceAgreed}) async {
    final payload = {
    ...
    final response = await _signedRequest(newAccountUrl, payload, null, keys: keys);
    var body = await _readBodyAsString(response);
  ...
  Future<AcmeOrder> newOrder(AcmeAccount account, List<String> hosts) async {
    final payload = {
      "identifiers": hosts.map((domain) => {"type": "dns", "value": domain}).toList()
    };
  ...
  Future<AcmeAuthorization> authorization(AcmeAccount account, String authorizationUrl) async {
  ...
  Future<AcmeChallenge> challengeComplete(AcmeAccount account, AcmeChallenge challenge) async {
    final response = await _signedRequest(challenge.url, <String, dynamic>{}, account);
    var body = await _readBodyAsString(response);
  ...
  Future<AcmeChallenge> challengeCheck(AcmeAccount account, AcmeChallenge challenge) async {
  ...
  Future<AcmeOrder> finalize(AcmeAccount account, AcmeOrder order, String csrPem) async {
    final csr = pemToDer(csrPem);
    final payload = {"csr": _base64Bytes(csr)};
    var response = await _signedRequest(order.finalize, payload, account, contentType: 'application/pem-csr');
    var body = await _readBodyAsString(response);
  ...
  Future<AcmeDnsRecord> dnsRecord(AcmeAccount account, AcmeChallenge dns, String host) async {
    ...
    var content = _base64Bytes(sha256.convert(utf8.encode(rawChallengeResponse)).bytes);
    return AcmeDnsRecord(name, content);
  }

  Future<HttpClientResponse> _signedRequest(String url, Map<String, dynamic>? payload, AcmeAccount? account,
      {AsymmetricKeyPair<RSAPublicKey, RSAPrivateKey>? keys, String contentType = 'application/jose+json'}) async {
    ...
    final protectedHeader = {
      ...
      "nonce": nonce,
      "url": url,
      if (locationUrl != null) "kid": locationUrl else "jwk": await _getJwk(keys!.publicKey)
    };
    ...
    final body = {"protected": protected64, "payload": payload64, "signature": signature};
  ...
  Future<String> _getNonce() async {
    final response = await (await client.headUrl(Uri.parse(newNonceUrl))).close();
  ...
  static String _thumbprint(AcmeJwk jwk) {
    ...
    return _base64Bytes(sha256.convert(utf8.encode(_sortedJsonEncode(jwk.toJson()))).bytes);
  }
  ...
  static Uint8List pemToDer(String pem) {
    final base64String = LineSplitter().convert(pem).where((line) => !line.startsWith('-----')).join('');
...
class AcmeOrder {
  ...
  factory AcmeOrder.fromJson(Map<String, dynamic> json) {
    return AcmeOrder(
      ...
      identifiers: (json['identifiers'] as List).map((i) => AcmeIdentifier.fromJson(i)).toList(),
...
class AcmeAuthorization {
  ...
  factory AcmeAuthorization.fromJson(Map<String, dynamic> json) {
    return AcmeAuthorization(
      ...
      status: json['status'],
      expires: DateTime.parse(json['expires']),
      challenges: (json['challenges'] as List).map((c) => AcmeChallenge.fromJson(c)).toList(),
    );
  }
}
...
class AcmeAccount {
  ...
  factory AcmeAccount._fromJsonEtc(
      Map<String, dynamic> json, String locationUrl, AsymmetricKeyPair<RSAPublicKey, RSAPrivateKey> keys) {
    return AcmeAccount(
  ...
  factory AcmeAccount.fromJson(Map<String, dynamic> json) {
    return AcmeAccount(
      ...
      initialIp: json['initialIp'],
      createdAt: DateTime.parse(json['createdAt']),
      status: AcmeStatus.values.firstWhere((e) => e.toString() == 'AcmeStatus.${json['status']}'),
...
///  });
/// ```
Future<T> _withCleanup<T>(Future<T> Function(void Function(FutureOr<void> Function()) defer) fn) async {
